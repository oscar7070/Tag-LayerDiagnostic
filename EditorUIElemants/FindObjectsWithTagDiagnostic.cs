...
public class FindObjectsWithTagDiagnostic : EditorWindow {
	...
	void OnGUI ()
	{
		...
		if (Russian == false) {
			...
			GUILayout.Label ("Created by oscar7070(Ver 1.2 Data 24/01/2020)for unity 2017.4))");
		} else 
		{
			options [0] = EditorGUILayout.Toggle ("Искать Тег :",options[0]);
			options [1] = EditorGUILayout.Toggle ("Искать Слой :",options[1]);
			GUILayout.Label ("Создано oscar7070(Версия 1.2 Дата 24/01/2020)дла unity 2017.4))");
		}
	}
	#endif
}